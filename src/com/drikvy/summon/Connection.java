...
private int port = 0;
  ...
  private ArrayList<MarkerOptions> events = new ArrayList<MarkerOptions>();
  ...
  public void addEventAction(String actionEvent) {
	  Log.d("Connection", "Incoming message: "+actionEvent);
	  String convJson = actionEvent.substring(actionEvent.indexOf("{", 0)
			  , actionEvent.indexOf("}", 0)+1);
	  Log.d("Connection", "convert:: "+convJson);
	  try {
		JSONObject objEvent = new JSONObject(convJson);
		
		events.add(new MarkerOptions()
		.position(new LatLng(Double.parseDouble(objEvent.getString("lat"))
				, Double.parseDouble(objEvent.getString("lng"))))
		.title(objEvent.getString("title"))
		.snippet(objEvent.getString("desc"))
		.icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_RED)));
		
		notifyListeners(new PropertyChangeEvent(this, ActivityConstants.eventProperty, null, null));
  ...
  public MarkerOptions[] event() {
	  int i = 0;
	  MarkerOptions[] array = new MarkerOptions[events.size()];

	    for (MarkerOptions mo : events) {
	      if (mo != null) {
	        array[i] = mo;
	        i++;
	      }
